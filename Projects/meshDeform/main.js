var DEFAULT_OBJECT = window.location.hash.length > 1 ?
    window.location.hash.substr(1) : getParameterByName('obj') || 'iris';

var container, stats;
var time = 0; 
var camera, scene, renderer, manager, controls, vDistance;
  ...
  loader.load('obj/' + name + '.txt', function(object) {
    object.traverse(function(child) {
      if (child instanceof THREE.Mesh) {
        var material = new THREE.MeshLambertMaterial({color: 0x002233});
...
function render() {
  ...
  //  obj.rotation.x %= 360;
     ...
     geo = obj.children[0];
     me = new THREE.Geometry().fromBufferGeometry( geo.geometry );

     me.mergeVertices();
   //  me.vertices[0].x += time/300;
   move = geo.geometry.boundingBox.min.z*Math.sin(8*time%360/180*Math.PI);
   refer = 0.6*(geo.geometry.boundingBox.min.z + geo.geometry.boundingBox.max.z);
   for(var i = 0; i < me.vertices.length; i++){
    temp = 0.2*(me.vertices[i].y - refer);
    me.vertices[i].y += Math.abs(temp*Math.sin(4*time % 360 / 180 * Math.PI));
   }
     mesh = new THREE.Mesh( me, material );
     mesh.translateY(move);
